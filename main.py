...
# ── Kotak Instrument Tokens (V2 Specifications) ──────────────────────────────
INSTRUMENT_TOKENS = {
...
# ── V2 COMPLIANT QUOTE RETRIEVAL HELPERS ──────────────────────────────────────
async def _get_quote_item(c: httpx.AsyncClient, sess: dict, neo_symbol: str, exch: str):
    """Retrieve quote items safely via V2 REST script-details endpoints."""
...
# ── NEWS SENTIMENT DICTIONARY LEXICON DEFINITIONS ─────────────────────────────
...
#### Copy and paste **PART 4** into your terminal prompt, then press Enter:
```bash
cat << 'EOF' > /root/Kotak-trading-bot/main_part4.py
# ── GLOBAL NEWS LEXICON DICTIONARIES ──────────────────────────────────────────
_BULL_WORDS = {"surge", "rally", "gain", "gains", "upgrade", "beat", "beats", "record",
               "high", "strong", "rises", "jumps", "jump", "soars", "soar", "outperform",
               "buy", "positive", "bullish", "accelerate", "recover", "recovery"}
_BEAR_WORDS = {"plunge", "fall", "falls", "drop", "drops", "downgrade", "miss", "misses",
...
#### Copy and paste **PART 5** into your terminal prompt, then press Enter:
*(Ensure you hit Enter at the end of the paste so the terminal runs the `EOF` delimiter and writes the file).*
```bash
cat << 'EOF' > /root/Kotak-trading-bot/main_part5.py
_BULL_WORDS = {"surge", "rally", "gain", "gains", "upgrade", "beat", "beats", "record",
               "high", "strong", "rises", "jumps", "jump", "soars", "soar", "outperform",
               "buy", "positive", "bullish", "accelerate", "recover", "recovery"}
_BEAR_WORDS = {"plunge", "fall", "falls", "drop", "drops", "downgrade", "miss", "misses",
