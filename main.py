...
# ── FULL MARKET DATA (LTP + OHLC + OI + VIX in one call) ────────────────────
@app.post("/quotes/market_data")
async def get_market_data(req: QuoteRequest):
    """Fetches LTP, OHLC, OI for the instrument + VIX in one combined call"""
    sess = get_session(req.session_id)
    inst = INSTRUMENT_TOKENS.get(req.instrument.upper())
    ...
    async with httpx.AsyncClient(timeout=20) as c:
        try:
            # Fetch instrument + VIX together
            payload = {
                "instrument_tokens": [inst, vix],
                "quote_type": "",   # empty = full data
                "isIndex": True
            }
            r = await c.post(
                f"{sess['base_url']}/quick/quotes",
                ...
                json=payload)
            raw = r.json()
            logger.info(f"Market data raw: {str(raw)[:300]}")

            # Parse response — Kotak returns list under "message" key
            items = []
            if isinstance(raw, dict):
                items = raw.get("message", raw.get("data", []))
            elif isinstance(raw, list):
                items = raw

            result = {"instrument": req.instrument, "success": True}
            for item in items:
                if not isinstance(item, dict): continue
                token = str(item.get("instrument_token", ""))
                ltp   = item.get("last_traded_price", item.get("ltp", "0"))
                ohlc  = item.get("ohlc", {})
                chng  = item.get("net_change_percentage", item.get("change_percentage", "0"))
                oi    = item.get("open_interest", "0")
                vol   = item.get("volume", "0")

                if token == inst["instrument_token"]:
                    result["ltp"]    = float(ltp or 0)
                    result["open"]   = float(ohlc.get("open", 0) if ohlc else 0)
                    result["high"]   = float(ohlc.get("high", 0) if ohlc else 0)
                    result["low"]    = float(ohlc.get("low", 0) if ohlc else 0)
                    result["close"]  = float(ohlc.get("close", 0) if ohlc else 0)
